...
import { apiGetFundraiserDetails } from '@/api.js';
  ...
  const handleStartSession = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/start-session', {
        ...
        body: JSON.stringify({ player_id: playerData.player_id })
      });
      ...
      alert('Session started! The tracker window should now be open.');
    } catch (err) {
    ...
    <div className="fundraiser-detail-page">
      ...
      <div className="fundraiser-stats-grid">
        <div className="stat-card">
          <h4>Amount Raised</h4>
          <p className="stat-value">${fundraiser.amount_raised.toFixed(2)}</p>
        </div>
        <div className="stat-card">
          <h4>Goal</h4>
          <p className="stat-value">${fundraiser.goal_amount.toFixed(2)}</p>
        </div>
        <div className="stat-card">
          <h4>Total Putts Made</h4>
          <p className="stat-value">{fundraiser.total_putts_made}</p>
        </div>
        <div className="stat-card">
          <h4>Total Pledged Per Putt</h4>
          <p className="stat-value">${totalPledgePerPutt.toFixed(2)}</p>
        </div>
      </div>
      ...
      <div className="card chart-card">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={chartData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
            ...
            <XAxis dataKey="putts" name="Putts Made" label={{ value: 'Putts Made', position: 'insideBottom', offset: -5 }} />
            <YAxis label={{ value: 'Amount Raised ($)', angle: -90, position: 'insideLeft' }} />
            <Tooltip formatter={(value) => `$${value.toFixed(2)}`} />
            <Legend />
            <Line type="monotone" dataKey="amount" name="Amount Raised" stroke="#8884d8" activeDot={{ r: 8 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="fundraiser-columns">
          ...
          {fundraiser.pledges.length > 0 ? (
            <table className="pledges-table">
              <thead>
                <tr>
                  <th>Pledger</th>
                  <th>Amount per Putt</th>
                </tr>
              </thead>
              <tbody>
                {fundraiser.pledges.map(pledge => (
                  <tr key={pledge.pledge_id}>
                    <td>{pledge.pledger_name}</td>
                    <td>${pledge.amount_per_putt.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card sessions-card">
          ...
          {fundraiser.sessions.length > 0 ? (
            <ul>
              {fundraiser.sessions.map(session => (
                <li key={session.session_id}>
                  <span>{new Date(session.start_time).toLocaleDateString()}</span>
                  <span>{session.total_makes} Makes</span>
                  <span>{Math.round(session.session_duration / 60)} min</span>
                </li>
              ))}
            </ul>
          ) : (
