...
import { apiGetLeagueDetails, apiInviteToLeague, apiJoinLeague, apiStartSession } from '@/api.js';
...
const LeagueDetailPage = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      setIsLoading(true);
      try {
        const data = await apiGetLeagueDetails(leagueId);
        setLeague(data);
      } catch (err) {
  ...
  const handleLeagueUpdated = () => {
    ...
    // Refetch details to show updated schedule
    const fetchDetails = async () => {
      const data = await apiGetLeagueDetails(leagueId);
      setLeague(data);
    };
  ...
  const handleJoinLeague = async () => {
    ...
    try {
      ...
      // Refetch details to update member list and button visibility
      const data = await apiGetLeagueDetails(leagueId);
      setLeague(data);
    } catch (err) {
  ...
  const handleStartLeagueSession = async (roundId) => {
    try {
      ...
      alert(`Failed to start league session: ${err.message}`);
    }
  };
  ...
  const formatRoundDuration = (hours) => {
    if (!hours) return 'N/A';
    if (hours < 24) {
  ...
  // All data processing is moved into useMemo hooks to prevent re-calculation on every render
  // and to fix the "Hooks called in a different order" error.
  ...
  const handleSort = (type, id = null) => {
    setSortOrder({ type, id });
  };

  const resetSort = () => setSortOrder({ type: 'default' });
  ...
  const canInvite = league.creator_id === playerData.player_id ||
    ...
    (isMember && league.settings.members_can_invite);
    ...
    <div className="league-detail-page">
      ...
      <div className="card league-info-card">
        <div className="section-header">
          ...
          {canEdit && <button className="btn-tertiary btn-small" onClick={() => setShowEditModal(true)}>Edit Schedule</button>}
        </div>
        ...
        <div className="league-info-grid">
          ...
          <table className="league-info-table">
            ...
            <tbody>
              <tr>
                <td>Time Limit</td>
                <td>{league.settings.time_limit_minutes} mins</td>
              </tr>
              <tr>
                <td>Rounds</td>
              ...
              <tr>
                ...
                <td>{league.settings.members_can_invite ? 'Allowed' : 'Not Allowed'}</td>
              </tr>
            </tbody>
          </table>
        </div>
