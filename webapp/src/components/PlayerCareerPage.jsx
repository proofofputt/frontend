...
const PlayerCareerPage = () => {
  ...
  const { is_subscribed, player_name } = stats;
    ...
    <div className="career-stats-page">
      ...
      <div className="stats-split-grid">
        <table className="career-stats-table">
          ...
          <tbody>
            <StatRow label="Makes" best={stats.high_makes} cumulative={stats.sum_makes} isSubscribed={is_subscribed} />
            <StatRow label="Best Streak" best={stats.high_best_streak} cumulative="-" isSubscribed={is_subscribed} />
            <StatRow label="Fastest 21" best={stats.low_fastest_21} cumulative="-" isSubscribed={is_subscribed} unit="s" />
            <StatRow label="Most in 60s" best={stats.high_most_in_60} cumulative="-" isSubscribed={is_subscribed} />
            <StatRow label="Putts Per Minute" best={stats.high_ppm} cumulative={stats.avg_ppm} isSubscribed={is_subscribed} />
            <StatRow label="Makes Per Minute" best={stats.high_mpm} cumulative={stats.avg_mpm} isSubscribed={is_subscribed} />
            <StatRow label="Session Duration" best={stats.high_duration / 60} cumulative={stats.sum_duration / 60} isSubscribed={is_subscribed} unit="m" />
          </tbody>
        </table>
        ...
        <table className="career-stats-table">
          ...
          <tbody>
            {Object.entries(stats.consecutive).map(([cat, data]) => (
              <StatRow key={cat} label={`Streak of ${cat}`} best={data.high} cumulative={data.sum} isSubscribed={is_subscribed} />
            ))}
          </tbody>
        </table>
        ...
        <table className="career-stats-table">
          ...
          <tbody>
            ...
            {Object.entries(stats.makes_overview).sort(([,a],[,b]) => b.sum - a.sum).map(([cat, data]) => (
              <StatRow key={cat} label={cat} best={data.high} cumulative={data.sum} isSubscribed={is_subscribed} />
            ))}
            <tr className="table-category-header"><td colSpan="3">Detailed</td></tr>
            {Object.entries(stats.makes_detailed).sort(([,a],[,b]) => b.sum - a.sum).map(([cat, data]) => (
              <StatRow key={cat} label={cat} best={data.high} cumulative={data.sum} isSubscribed={is_subscribed} />
            ))}
          </tbody>
        </table>
        ...
        <table className="career-stats-table">
          ...
          <tbody>
            <tr className="table-category-header"><td colSpan="3">Overview</td></tr>
            {Object.entries(stats.misses_overview).sort(([,a],[,b]) => b.sum - a.sum).map(([cat, data]) => (
              <StatRow key={cat} label={cat.replace(/_/g, ' ')} best={data.high} cumulative={data.sum} isSubscribed={is_subscribed} />
            ))}
            <tr className="table-category-header"><td colSpan="3">Detailed</td></tr>
            {Object.entries(stats.misses_detailed).sort(([,a],[,b]) => b.sum - a.sum).map(([cat, data]) => (
              <StatRow key={cat} label={cat.replace(/_/g, ' ')} best={data.high} cumulative={data.sum} isSubscribed={is_subscribed} />
            ))}
          </tbody>
        </table>
